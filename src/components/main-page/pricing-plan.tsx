...
export const PricingPlan = (props: PricingPlanData) => {
  const { icon: Icon, popularChoice = false, title, price, pricePeriod, description, perks } = props;

  return (
    <div className={`relative h-fit w-full flex flex-col gap-4 ${popularChoice ? "border-3 border-primary-200 pt-12" : "border border-gray-4"} rounded-3xl p-6`}>
      {
        popularChoice &&
      ...
      <ul className="flex flex-col gap-2 text-gray-7">
        {
      ...
      <button className={`${popularChoice && "bg-primary-200"} w-full border-2 font-medium border-primary-200 rounded-full p-3`}>
        Get Started
      </button>
    </div>
  );
